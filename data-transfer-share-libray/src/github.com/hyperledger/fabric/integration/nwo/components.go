...
func (c *Components) Build(args ...string) {
	...
	cryptogen, err := gexec.Build("github.com/hyperledger/fabric/common/chaincode-tools/cryptogen", args...)
	Expect(err).NotTo(HaveOccurred())
	c.Paths["cryptogen"] = cryptogen

	idemixgen, err := gexec.Build("github.com/hyperledger/fabric/common/chaincode-tools/idemixgen", args...)
	Expect(err).NotTo(HaveOccurred())
	c.Paths["idemixgen"] = idemixgen

	configtxgen, err := gexec.Build("github.com/hyperledger/fabric/common/chaincode-tools/configtxgen", args...)
	Expect(err).NotTo(HaveOccurred())
