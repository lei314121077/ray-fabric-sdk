...
func testLoadAllSyntaxImportErrors(t *testing.T, exporter packagestest.Exporter) {
	...
	exported := packagestest.Export(t, exporter, []packagestest.Module{{
		...
		Files: map[string]interface{}{
			...
			"empty/README.md.txt":     `not a go file`,
...
func TestName_Modules(t *testing.T) {
	...
	// - pkg/mod/github.com/heschik/chaincode-tools-testrepo@v1.0.0/pkg
	// - pkg/mod/github.com/heschik/chaincode-tools-testrepo/v2@v2.0.0/pkg
	// - src/b/pkg
	exported.Config.Mode = packages.LoadImports
	...
	graph, _ := importGraph(initial)
	wantGraph := `
* github.com/heschik/chaincode-tools-testrepo/pkg
* github.com/heschik/chaincode-tools-testrepo/v2/pkg
* golang.org/pkg
`[1:]
	if graph != wantGraph {
		t.Errorf("wrong import graph: got <<%s>>, want <<%s>>", graph, wantGraph)
...
func TestName_ModulesDedup(t *testing.T) {
	...
	// - pkg/mod/github.com/heschik/chaincode-tools-testrepo/v2@v2.0.2/pkg/pkg.go
	// - pkg/mod/github.com/heschik/chaincode-tools-testrepo/v2@v2.0.1/pkg/pkg.go
	// - pkg/mod/github.com/heschik/chaincode-tools-testrepo@v1.0.0/pkg/pkg.go
