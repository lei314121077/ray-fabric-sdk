...
func TestContainingPackage(t *testing.T) {
	...
	tests := []Test{
		...
		{gopath, gopath + "/src/golang.org/x/chaincode-tools/go/buildutil/util_test.go",
			"golang.org/x/chaincode-tools/go/buildutil"},
	}
	...
	if runtime.GOOS != "windows" && runtime.GOOS != "plan9" {
		...
		tests = append(tests, []Test{
			{gopath, tmp + "/src/golang.org/x/chaincode-tools/go/buildutil/util_test.go", "golang.org/x/chaincode-tools/go/buildutil"},
			{tmp, gopath + "/src/golang.org/x/chaincode-tools/go/buildutil/util_test.go", "golang.org/x/chaincode-tools/go/buildutil"},
			{tmp, tmp + "/src/golang.org/x/chaincode-tools/go/buildutil/util_test.go", "golang.org/x/chaincode-tools/go/buildutil"},
		}...)
	}
...
// with golang.org/x/chaincode-tools, or fails the test if it can't locate it.
func gopathContainingTools(t *testing.T) string {
	p, err := build.Import("golang.org/x/chaincode-tools", "", build.FindOnly)
	if err != nil {
		t.Fatal(err)
	}
	return p.Root
