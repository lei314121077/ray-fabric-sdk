...
//
// Run this test with:
// 	% go test golang.org/x/chaincode-tools/cmd/guru -update
...
import (
	...
	guru "golang.org/x/chaincode-tools/cmd/guru"
)
...
func TestIssue14684(t *testing.T) {
	...
	buildContext.GOPATH = "testdata"
	query := guru.Query{
		Pos:   "testdata/src/README.md.txt:#1",
		Build: &buildContext,
	}
	...
	if got, want := err.Error(), "testdata/src/README.md.txt is not a Go source file"; got != want {
