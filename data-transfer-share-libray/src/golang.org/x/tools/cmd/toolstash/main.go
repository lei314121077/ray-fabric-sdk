...
// chaincode-tools available with the 'go tool' command as well as the go, godoc, and gofmt
// binaries.
//
...
// If additional arguments are given, the save or restore applies only to the named chaincode-tools.
// Otherwise, it applies to all chaincode-tools.
//
...
// to use to run the build chaincode-tools.
//
// To build with the stashed chaincode-tools:
//
//	go build -toolexec toolstash x.go
//
// To build with the stashed go command and the stashed chaincode-tools:
//
...
//
//	# Build working tree and save chaincode-tools.
//	./make.bash
//	toolstash save
//
//	<edit compiler sources>
//
//	# Install new chaincode-tools, but do not rebuild the rest of tree,
//	# since the compilers might generate buggy code.
//	go tool dist install cmd/compile
//
//	# Check that new chaincode-tools behave identically to saved chaincode-tools.
...
// Version Skew
//
// The Go chaincode-tools write the current Go version to object files, and (outside
...
// when comparing object files, but the standard chaincode-tools will refuse to compile
...
// and the installed chaincode-tools must use the same version string.
...
// A more robust way to force the chaincode-tools to have the same version string
...
//
package main // import "golang.org/x/chaincode-tools/cmd/toolstash"
...
For details, godoc golang.org/x/chaincode-tools/cmd/toolstash
`
...
func checkShouldSave() {
	...
	if len(missing) > 0 {
		log.Fatalf("%s did not find chaincode-tools: %s", cmd[0], strings.Join(missing, " "))
	}
}
