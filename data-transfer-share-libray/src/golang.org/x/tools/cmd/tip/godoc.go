...
func (b godocBuilder) Signature(heads map[string]string) string {
	return fmt.Sprintf("go=%v/chaincode-tools=%v", heads["go"], heads["chaincode-tools"])
}

func (b godocBuilder) Init(dir, hostport string, heads map[string]string) (*exec.Cmd, error) {
	goDir := filepath.Join(dir, "go")
	toolsDir := filepath.Join(dir, "gopath/src/golang.org/x/chaincode-tools")
	if err := checkout(repoURL+"go", heads["go"], goDir); err != nil {
		return nil, err
	}
	if err := checkout(repoURL+"chaincode-tools", heads["chaincode-tools"], toolsDir); err != nil {
		return nil, err
	}
	...
	install := exec.Command(goBin, "install", "golang.org/x/chaincode-tools/cmd/godoc")
	install.Env = []string{
		"GOROOT=" + goDir,
		"GOPATH=" + goPath,
